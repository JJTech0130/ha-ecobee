...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    auth0_token = entry.data[CONF_AUTH0_TOKEN]

    data = EcobeeData(hass, entry, api_key=api_key, refresh_token=refresh_token, auth0_token=auth0_token)
...
class EcobeeData:
    ...
    def __init__(
        self, hass: HomeAssistant, entry: ConfigEntry, api_key: str, refresh_token: str, auth0_token: str
    ) -> None:
        """Initialize the Ecobee data object."""
        self._hass = hass
        self.entry = entry
        self.ecobee = Ecobee(
            config={ECOBEE_API_KEY: api_key, ECOBEE_REFRESH_TOKEN: refresh_token, ECOBEE_AUTH0_TOKEN: auth0_token}
        )
        ...
        if await self._hass.async_add_executor_job(self.ecobee.refresh_tokens):
            self._hass.config_entries.async_update_entry(
                ...
                data={
                    ...
                    CONF_AUTH0_TOKEN: self.ecobee.config[ECOBEE_AUTH0_TOKEN],
                },
            )
            return True
